...
function renderIssue(issue: GitHubIssue): string {
  ...
  return `<details class="issue"><summary><span class="number">#${issue.number}</span><span class="title">${escapeHtml(issue.title)}</span><time class="updated" datetime="${escapeHtml(issue.updatedAt)}">${escapeHtml(formatDate(issue.updatedAt))}</time></summary><div class="actions"><button data-start-new="${escapeHtml(key)}">新規セッション</button><button class="secondary" data-attach="${escapeHtml(key)}">既存セッションへ渡す</button><button class="secondary" data-open="${escapeHtml(key)}">GitHubで開く</button><button class="secondary" data-copy-link="${escapeHtml(key)}" aria-label="Issue #${issue.number} のリンクをコピー">リンクをコピー</button></div><div class="meta">${labels}${assignees}</div><pre class="body">${escapeHtml(issue.body || "本文はありません。")}</pre></details>`;
}
