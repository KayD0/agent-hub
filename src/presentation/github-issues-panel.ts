...
export class GitHubIssuesPanel implements vscode.Disposable {
  ...
  public constructor(
    ...
    private readonly startIssueSession: (issue: GitHubIssue, groupId: string, target: "new" | "existing") => Promise<void>,
  ...
  private async handleMessage(groupId: string, value: unknown): Promise<void> {
    ...
    const message = value as { type?: unknown; issueKey?: unknown; issueKeys?: unknown };
    try {
      if (message.type === "refresh") {
        const panel = this.panels.get(groupId);
        if (panel) await this.renderOnce(groupId, panel);
      } else if ((message.type === "startNewIssue" || message.type === "attachIssue") && typeof message.issueKey === "string") {
        const issue = this.issues.get(`${groupId}:${message.issueKey}`);
        if (issue) await this.startIssueSession(issue, groupId, message.type === "startNewIssue" ? "new" : "existing");
      } else if (message.type === "bulkStartIssues" && Array.isArray(message.issueKeys)) {
        const keys = [...new Set(message.issueKeys.filter((key): key is string => typeof key === "string"))];
        const selected = keys.flatMap((key) => {
          const issue = this.issues.get(`${groupId}:${key}`);
          return issue ? [issue] : [];
        });
        let started = 0;
        let failed = 0;
        try {
          for (const issue of selected) {
            try { await this.startIssueSession(issue, groupId, "new"); started += 1; }
            catch (error) { failed += 1; this.onError(error); }
          }
        } finally {
          const panel = this.panels.get(groupId);
          void panel?.webview.postMessage({ type: "bulkStartComplete", started, failed });
        }
        void vscode.window.showInformationMessage(`${started}件のIssueで新規セッションを開始しました${failed ? `（${failed}件失敗）` : ""}。`);
      ...
      } else if (message.type === "copyIssueLink" && typeof message.issueKey === "string") {
        const issue = this.issues.get(`${groupId}:${message.issueKey}`);
        if (issue) {
          await vscode.env.clipboard.writeText(issue.url);
          await vscode.window.showInformationMessage(`Issue #${issue.number} のリンクをコピーしました。`);
        }
      }
...
function renderHtml(groupName: string, rootPath: string, results: GitHubIssueRepositoryResult[]): string {
  ...
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}'; script-src 'nonce-${nonce}';"><style nonce="${nonce}">*{box-sizing:border-box}body{margin:0;color:var(--vscode-foreground);background:var(--vscode-editor-background);font-family:var(--vscode-font-family)}header{position:sticky;top:0;z-index:2;display:flex;align-items:center;gap:12px;padding:14px 18px;border-bottom:1px solid var(--vscode-panel-border);background:var(--vscode-editor-background)}header .heading{min-width:0;flex:1}header .header-actions{display:flex;flex:none;gap:6px}h1{margin:0;font-size:18px}header p{margin:4px 0 0;overflow:hidden;color:var(--vscode-descriptionForeground);text-overflow:ellipsis;white-space:nowrap}button{border:0;padding:6px 10px;color:var(--vscode-button-foreground);background:var(--vscode-button-background);cursor:pointer}button:disabled{opacity:.55;cursor:default}button.secondary{color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground)}main{display:grid;gap:18px;padding:16px}h2{margin:0 0 8px;font-size:14px}.issue{display:grid;grid-template-columns:auto minmax(0,1fr) auto;margin-bottom:8px;border:1px solid var(--vscode-panel-border);border-radius:4px}.issue-selection{display:grid;padding:10px 2px 10px 10px;place-items:start center}.issue-selection input{margin:2px 0 0}.issue details{min-width:0}.issue summary{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:8px;padding:10px;cursor:pointer}.issue summary:hover,.issue summary:focus-visible{background:var(--vscode-list-hoverBackground);outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.number{color:var(--vscode-descriptionForeground)}.title{overflow:hidden;font-weight:600;text-overflow:ellipsis;white-space:nowrap}.updated{color:var(--vscode-descriptionForeground);font-size:11px}.issue-content{grid-column:1/-1;border-top:1px solid var(--vscode-panel-border)}.issue-content[hidden]{display:none}.meta{display:flex;flex-wrap:wrap;gap:6px;padding:8px 10px;color:var(--vscode-descriptionForeground);font-size:11px}.label{padding:2px 6px;border:1px solid var(--vscode-panel-border);border-radius:9px}.body{margin:0;padding:10px;border-top:1px solid var(--vscode-panel-border);white-space:pre-wrap;word-break:break-word;max-height:320px;overflow:auto}.actions{display:flex;align-items:center;justify-content:flex-end;gap:6px;padding:6px;border-left:1px solid var(--vscode-panel-border);white-space:nowrap}.actions button{padding:5px 8px}.message{padding:10px;border:1px solid var(--vscode-panel-border);color:var(--vscode-descriptionForeground)}.error{border-color:var(--vscode-inputValidation-errorBorder);color:var(--vscode-errorForeground)}@media(max-width:760px){header{align-items:flex-start;flex-wrap:wrap}header .header-actions{width:100%}.issue{grid-template-columns:auto minmax(0,1fr)}.actions{grid-column:1/-1;justify-content:flex-start;border-top:1px solid var(--vscode-panel-border);border-left:0;overflow-x:auto}.issue-content{grid-column:1/-1}.issue summary{grid-template-columns:auto minmax(0,1fr)}.updated{grid-column:2}}</style></head><body><header><div class="heading"><h1>${escapeHtml(groupName)} Issues</h1><p title="${escapeHtml(rootPath)}">Open ${issueCount}件・${escapeHtml(rootPath)}</p></div><div class="header-actions"><button id="bulk-start" disabled>一括作業開始 (0)</button><button id="refresh" class="secondary">更新</button></div></header><main>${content}</main><script nonce="${nonce}">const vscode=acquireVsCodeApi();const selections=[...document.querySelectorAll('[data-select-issue]')];const bulkStart=document.getElementById('bulk-start');let bulkStarting=false;const updateBulkStart=()=>{const count=selections.filter(input=>input.checked).length;bulkStart.textContent='一括作業開始 ('+count+')';bulkStart.disabled=bulkStarting||count===0;};selections.forEach(input=>input.addEventListener('change',updateBulkStart));bulkStart.addEventListener('click',()=>{const issueKeys=selections.filter(input=>input.checked).map(input=>input.dataset.selectIssue);if(!issueKeys.length||bulkStarting)return;bulkStarting=true;updateBulkStart();vscode.postMessage({type:'bulkStartIssues',issueKeys});});window.addEventListener('message',event=>{if(event.data?.type!=='bulkStartComplete')return;bulkStarting=false;selections.forEach(input=>input.checked=false);updateBulkStart();});document.getElementById('refresh').addEventListener('click',()=>vscode.postMessage({type:'refresh'}));document.querySelectorAll('.issue details').forEach(details=>details.addEventListener('toggle',()=>{const content=details.parentElement.querySelector('.issue-content');if(content)content.hidden=!details.open;}));document.querySelectorAll('[data-start-new]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'startNewIssue',issueKey:button.dataset.startNew})));document.querySelectorAll('[data-attach]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'attachIssue',issueKey:button.dataset.attach})));document.querySelectorAll('[data-open]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'openIssue',issueKey:button.dataset.open})));document.querySelectorAll('[data-copy-link]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'copyIssueLink',issueKey:button.dataset.copyLink})));</script></body></html>`;
}
...
function renderIssue(issue: GitHubIssue): string {
  ...
  return `<article class="issue"><label class="issue-selection" title="Issue #${issue.number}を一括作業開始の対象にする"><input type="checkbox" data-select-issue="${escapeHtml(key)}" aria-label="Issue #${issue.number}を選択"></label><details><summary><span class="number">#${issue.number}</span><span class="title">${escapeHtml(issue.title)}</span><time class="updated" datetime="${escapeHtml(issue.updatedAt)}">${escapeHtml(formatDate(issue.updatedAt))}</time></summary></details><div class="actions"><button data-start-new="${escapeHtml(key)}">新規セッション</button><button class="secondary" data-attach="${escapeHtml(key)}">既存セッションへ渡す</button><button class="secondary" data-open="${escapeHtml(key)}">GitHubで開く</button><button class="secondary" data-copy-link="${escapeHtml(key)}" aria-label="Issue #${issue.number} のリンクをコピー">リンクをコピー</button></div><div class="issue-content" hidden><div class="meta">${labels}${assignees}</div><pre class="body">${escapeHtml(issue.body || "本文はありません。")}</pre></div></article>`;
}
