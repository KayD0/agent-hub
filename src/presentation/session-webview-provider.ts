...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    .empty{color:var(--vscode-descriptionForeground);padding:16px 4px}.group{margin-top:12px}.group-title{display:flex;align-items:center;gap:6px;margin:0 0 5px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--vscode-sideBarSectionHeader-foreground)}
    .count{padding:1px 5px;border-radius:8px;background:var(--vscode-badge-background);color:var(--vscode-badge-foreground)}.session{border:1px solid var(--vscode-sideBar-border,var(--vscode-panel-border));border-radius:4px;margin-bottom:7px;padding:8px;background:var(--vscode-sideBar-background)}
    ...
    button.send,.actions button{border:0;padding:4px 8px;background:var(--vscode-button-background);color:var(--vscode-button-foreground);cursor:pointer}button.send:hover,.actions button:hover{background:var(--vscode-button-hoverBackground)}.actions{display:flex;flex-wrap:wrap;gap:5px;margin-top:7px}.actions button.secondary{background:var(--vscode-button-secondaryBackground);color:var(--vscode-button-secondaryForeground)}
  </style>
</head>
...
<script nonce="${nonce}">
  ...
  const labels={starting:'開始中',running:'実行中',waiting_for_approval:'承認待ち',waiting_for_input:'入力待ち',completed:'完了',failed:'失敗',interrupted:'中断',disconnected:'切断'};
  ...
  function render(){const drafts=new Map([...document.querySelectorAll('textarea[data-session]')].map(x=>[x.dataset.session,x.value]));const active=document.activeElement?.dataset?.session;root.replaceChildren();if(!groups.length){root.append(el('p','empty','セッションはまだありません。'));return}
    for(const group of groups){const section=el('section','group');const heading=el('h2','group-title',group.label);heading.append(el('span','count',String(group.sessions.length)));section.append(heading);
      for(const session of group.sessions){const card=el('article','session');const head=el('div','session-head');const main=el('div','session-main');main.append(el('div','title',session.title));main.append(el('div','meta',session.cwd.split(/[\\/]/).pop()+' · '+labels[session.status]));if(session.currentActivity)main.append(el('div','activity',session.currentActivity));head.append(main,el('span','status',labels[session.status]));const open=button('↗','詳細を開く',()=>vscode.postMessage({type:'open',sessionId:session.id}));open.className='icon-button';head.append(open);card.append(head);
        const form=el('form');const input=el('textarea');input.dataset.session=session.id;input.rows=1;input.placeholder='このセッションへ指示...';input.setAttribute('aria-label',session.title+'への指示');input.value=drafts.get(session.id)||'';const send=el('button','send','送信');send.type='submit';form.append(input,send);form.addEventListener('submit',event=>{event.preventDefault();const text=input.value.trim();if(!text)return;vscode.postMessage({type:'send',sessionId:session.id,text});input.value=''});input.addEventListener('keydown',event=>{if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)){event.preventDefault();form.requestSubmit()}});card.append(form);
        ...
        if(['starting','running','waiting_for_input'].includes(session.status))actions.append(button('中断','処理を中断',()=>vscode.postMessage({type:'interrupt',sessionId:session.id}),true));if(['completed','failed','interrupted','disconnected'].includes(session.status))actions.append(button('削除','一覧から削除',()=>vscode.postMessage({type:'remove',sessionId:session.id}),true));if(actions.childElementCount)card.append(actions);section.append(card)}root.append(section)}
