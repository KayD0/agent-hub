...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    .app-header{position:sticky;z-index:30;top:0;padding-top:6px;background:var(--vscode-sideBar-background)}.auth{margin:0;padding:7px 8px;border:1px solid var(--vscode-sideBar-border,var(--vscode-panel-border));border-radius:4px}.auth.error{border-color:var(--vscode-inputValidation-errorBorder)}.auth.authenticated{display:flex;align-items:center;gap:8px;white-space:nowrap}.auth.authenticated .auth-title{flex:none}.auth.authenticated .auth-detail{min-width:0;flex:1;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.auth.authenticated .actions{flex:none;margin:0}.auth-title{font-weight:600}.auth-detail{margin:5px 0;color:var(--vscode-descriptionForeground);word-break:break-word}.auth-code{display:block;margin:8px 0;padding:7px;text-align:center;font:600 16px var(--vscode-editor-font-family);letter-spacing:2px;background:var(--vscode-textCodeBlock-background);user-select:all}.folder-filters{position:relative;margin:0 0 10px}.folder-filters summary{padding:7px 10px;border:1px solid var(--vscode-sideBar-border,var(--vscode-panel-border));border-top:0;border-radius:0 0 4px 4px;color:var(--vscode-descriptionForeground);font-size:11px;cursor:pointer;list-style-position:inside}.folder-filters[open] summary{border-radius:0}.folder-filters summary:hover,.folder-filters summary:focus-visible{background:var(--vscode-list-hoverBackground);outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.filter-options{position:absolute;z-index:20;right:0;left:0;display:grid;gap:6px;padding:8px 10px;border:1px solid var(--vscode-widget-border,var(--vscode-panel-border));border-top:0;background:var(--vscode-editorHoverWidget-background,var(--vscode-sideBar-background));box-shadow:0 2px 8px var(--vscode-widget-shadow)}.filter-actions{display:flex;gap:6px;padding-bottom:7px;border-bottom:1px solid var(--vscode-widget-border,var(--vscode-panel-border))}.filter-actions button{flex:1;padding:4px 6px;font-size:11px}.folder-filter{display:flex;align-items:center;gap:6px;min-width:0;font-size:11px}.folder-filter input{margin:0}.folder-filter span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .bulk-auto{display:flex;justify-content:flex-end;padding:5px 7px;border:1px solid var(--vscode-sideBar-border,var(--vscode-panel-border));border-top:0;font-size:11px}.bulk-auto label{display:flex;align-items:center;gap:4px;cursor:pointer}.bulk-auto input{margin:0}.bulk-auto:has(input:checked){color:var(--vscode-inputValidation-warningForeground,var(--vscode-charts-yellow));font-weight:600}
    .folder-filters summary{padding:3px 7px;font-size:10px;line-height:16px}
    .filter-actions button{flex:0 0 16%;min-width:64px;padding:3px;font-size:10px;white-space:nowrap}
    .empty{color:var(--vscode-descriptionForeground);padding:16px 4px}.sessions-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:7px;margin-top:12px}.session{min-width:0;border:1px solid var(--vscode-sideBar-border,var(--vscode-panel-border));border-left:3px solid var(--vscode-descriptionForeground);border-radius:4px;padding:8px;background:var(--vscode-sideBar-background);font-size:12px}
...
<script nonce="${nonce}">
  ...
  function decorateBulkAuto(){if(authentication.status!=='authenticated')return;const section=el('div','bulk-auto');const label=el('label');label.title='ポリシーに一致する承認要求だけを全セッションで自動許可';const input=el('input');input.id='bulk-auto';input.type='checkbox';const enabled=sessions.filter(session=>session.autoApprove).length;input.checked=sessions.length>0&&enabled===sessions.length;input.indeterminate=enabled>0&&enabled<sessions.length;input.disabled=sessions.length===0;input.setAttribute('aria-label','全セッションのポリシーAuto承認');input.addEventListener('change',()=>vscode.postMessage({type:'autoApproveAll',enabled:input.checked}));label.append(input,document.createTextNode('一括Auto'));section.append(label);root.prepend(section)}
  function renderFolderFilters(){for(const id of [...selectedRepositoryGroupIds])if(!repositoryGroups.some(group=>group.id===id))selectedRepositoryGroupIds.delete(id);const dropdown=el('details','folder-filters');dropdown.open=folderFilterOpen;dropdown.addEventListener('toggle',()=>{folderFilterOpen=dropdown.open});dropdown.setAttribute('aria-label','セッションのフォルダフィルター');const count=selectedRepositoryGroupIds.size;dropdown.append(el('summary','',count?'フォルダ（'+count+'件選択）':'フォルダ（すべて）'));const options=el('div','filter-options');const persistFilters=()=>{folderFilterOpen=true;vscode.setState({...vscode.getState(),repositoryGroupFilterIds:[...selectedRepositoryGroupIds]});renderSessions()};const filterActions=el('div','filter-actions');filterActions.append(button('すべてチェック','すべてのフォルダをチェック',()=>{for(const group of repositoryGroups)selectedRepositoryGroupIds.add(group.id);persistFilters()}),button('すべてクリア','すべてのフォルダチェックをクリア',()=>{selectedRepositoryGroupIds.clear();persistFilters()},true));options.append(filterActions);for(const group of repositoryGroups){const label=el('label','folder-filter');label.title=group.rootPath;const input=el('input');input.type='checkbox';input.checked=selectedRepositoryGroupIds.has(group.id);input.setAttribute('aria-label',group.name+'のセッションを表示');input.addEventListener('change',()=>{if(input.checked)selectedRepositoryGroupIds.add(group.id);else selectedRepositoryGroupIds.delete(group.id);persistFilters()});label.append(input,el('span','',group.name));options.append(label)}dropdown.append(options);return dropdown}
  ...
  function renderSessions(){const focus=captureFocus();const bulkAutoFocused=document.activeElement?.id==='bulk-auto';preserveHoveredResult();render();decorateBulkAuto();decorateResults();decorateAutoControls();restoreFocus(focus);if(bulkAutoFocused)document.getElementById('bulk-auto')?.focus()}
