...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    ...
    .last-instruction{display:-webkit-box;margin-top:6px;overflow:hidden;color:var(--vscode-descriptionForeground);font-size:11px;line-height:1.4;-webkit-box-orient:vertical;-webkit-line-clamp:3;line-clamp:3;white-space:normal;overflow-wrap:anywhere}
  </style>
</head>
...
<script nonce="${nonce}">
  ...
  function openResult(sessionId,popover){root.querySelector('.card-input-popover:focus-within textarea')?.blur();for(const timer of resultCloseTimers.values())clearTimeout(timer);for(const timer of inputCloseTimers.values())clearTimeout(timer);resultCloseTimers.clear();inputCloseTimers.clear();openResults.clear();root.querySelectorAll('.result-popover.is-open,.card-input-popover.is-open').forEach(node=>node.classList.remove('is-open'));openResults.add(sessionId);popover.classList.add('is-open')}
  ...
  function openCardInput(sessionId,popover){for(const timer of resultCloseTimers.values())clearTimeout(timer);for(const timer of inputCloseTimers.values())clearTimeout(timer);resultCloseTimers.clear();inputCloseTimers.clear();openResults.clear();root.querySelectorAll('.result-popover.is-open,.card-input-popover.is-open').forEach(node=>node.classList.remove('is-open'));popover.classList.add('is-open')}
