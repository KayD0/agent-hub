...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    ...
    .session-head{display:flex;align-items:flex-start;gap:6px}.session-main{min-width:0;flex:1}.title{font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.activity{margin-top:3px;color:var(--vscode-descriptionForeground);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.status{font-size:10px;padding:1px 5px;border:1px solid currentColor;border-radius:8px;color:var(--vscode-descriptionForeground)}.result{margin:8px 0 0;padding:8px;border-radius:3px;background:var(--vscode-textCodeBlock-background);white-space:pre-wrap;word-break:break-word;max-height:180px;overflow:auto}.result[hidden]{display:none}.result-toggle{margin-top:7px;border:0;padding:3px 6px;background:transparent;color:var(--vscode-descriptionForeground);cursor:pointer}.result-toggle:hover{background:var(--vscode-toolbar-hoverBackground);color:var(--vscode-foreground)}.result-label{display:block;margin-bottom:4px;color:var(--vscode-descriptionForeground);font-size:11px;font-weight:600}
...
<script nonce="${nonce}">
  const vscode=acquireVsCodeApi();const root=document.getElementById('sessions');const expandedResults=new Set();let sessions=[];let draggedSessionId;
  ...
  function decorateResults(){for(const session of sessions){const card=root.querySelector('[data-session-id="'+CSS.escape(session.id)+'"]');if(!card||!session.finalResult)continue;const resultId='result-'+session.id.replace(/[^a-zA-Z0-9_-]/g,'-');const expanded=expandedResults.has(session.id);const toggle=button(expanded?'最終結果を隠す':'最終結果を表示','最終結果を開閉',()=>{const willExpand=toggle.getAttribute('aria-expanded')!=='true';toggle.setAttribute('aria-expanded',String(willExpand));toggle.textContent=willExpand?'最終結果を隠す':'最終結果を表示';result.hidden=!willExpand;if(willExpand)expandedResults.add(session.id);else expandedResults.delete(session.id)},true);toggle.className='result-toggle';toggle.setAttribute('aria-expanded',String(expanded));toggle.setAttribute('aria-controls',resultId);const result=el('div','result');result.id=resultId;result.hidden=!expanded;result.setAttribute('role','region');result.setAttribute('aria-label','最終結果');result.append(el('span','result-label','最終結果'),document.createTextNode(session.finalResult));const form=card.querySelector('form');card.insertBefore(toggle,form);card.insertBefore(result,form)}}
