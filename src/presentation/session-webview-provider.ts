...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    ...
    .session-head{display:flex;align-items:flex-start;gap:6px}.session-main{min-width:0;flex:1}.title{font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.activity{margin-top:3px;color:var(--vscode-descriptionForeground);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.status{font-size:10px;padding:1px 5px;border:1px solid currentColor;border-radius:8px;color:var(--vscode-descriptionForeground)}.result{display:none;margin:8px 0 0;padding:8px;border-radius:3px;background:var(--vscode-textCodeBlock-background);white-space:pre-wrap;word-break:break-word;max-height:180px;overflow:auto}.session:focus,.session:focus-within{outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.session:focus .result,.session:focus-within .result{display:block}.result-label{display:block;margin-bottom:4px;color:var(--vscode-descriptionForeground);font-size:11px;font-weight:600}
...
<script nonce="${nonce}">
  ...
  function decorateResults(){for(const session of sessions){const card=root.querySelector('[data-session-id="'+CSS.escape(session.id)+'"]');if(!card)continue;card.tabIndex=0;card.setAttribute('aria-label',session.title+'。フォーカスすると最終結果を表示します');if(!session.finalResult)continue;const result=el('div','result');result.setAttribute('role','region');result.setAttribute('aria-label','最終結果');result.append(el('span','result-label','最終結果'),document.createTextNode(session.finalResult));card.insertBefore(result,card.querySelector('form'))}}
  window.addEventListener('message',event=>{if(event.data?.type==='sessions'){sessions=event.data.sessions;render();decorateResults()}});
