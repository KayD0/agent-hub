...
<script nonce="${nonce}">
  const vscode=acquireVsCodeApi();const root=document.getElementById('sessions');const openResults=new Set();const resultCloseTimers=new Map();let sessions=[];let authentication={status:'checking'};let draggedSessionId;let composingSessionId;let renderPending=false;
