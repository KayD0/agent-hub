...
<head>
  ...
  <style nonce="${nonce}">
    *{box-sizing:border-box}body{padding:0 8px 12px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}
    ...
    .drag-handle,.icon-button{flex:none;border:0;padding:2px 5px;background:transparent;color:var(--vscode-foreground)}.drag-handle{cursor:grab;user-select:none}.drag-handle:active{cursor:grabbing}.drag-handle:hover,.drag-handle:focus,.icon-button:hover{outline:none;background:var(--vscode-toolbar-hoverBackground)}.session.dragging{opacity:.55}form{display:flex;gap:5px;margin-top:8px}textarea{min-width:0;flex:1;resize:vertical;min-height:30px;max-height:100px;padding:5px 6px;border:1px solid var(--vscode-input-border,transparent);background:var(--vscode-input-background);color:var(--vscode-input-foreground);font:inherit}textarea:focus{outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.card-input-popover{position:relative;display:inline-block;margin-top:7px}.card-input-toggle{border:0;padding:3px 6px;background:transparent;color:var(--vscode-descriptionForeground);cursor:pointer}.card-input-toggle:hover,.card-input-toggle:focus{background:var(--vscode-toolbar-hoverBackground);color:var(--vscode-foreground)}.card-input-form{display:none;position:absolute;z-index:12;top:50%;left:calc(100% + 7px);width:min(320px,calc(100vw - 32px));margin:0;padding:8px;border:1px solid var(--vscode-widget-border,var(--vscode-panel-border));border-radius:4px;background:var(--vscode-editorHoverWidget-background,var(--vscode-sideBar-background));box-shadow:0 2px 8px var(--vscode-widget-shadow);transform:translateY(-50%)}.card-input-form::before{content:'';position:absolute;top:0;right:100%;bottom:0;width:8px}.card-input-popover:hover .card-input-form,.card-input-popover:focus-within .card-input-form{display:flex}.card-input-form textarea{min-height:60px}.card-input-form button{align-self:flex-end;border:0;padding:4px 8px;background:var(--vscode-button-background);color:var(--vscode-button-foreground);cursor:pointer}.card-input-form button:hover{background:var(--vscode-button-hoverBackground)}
...
<script nonce="${nonce}">
  const vscode=acquireVsCodeApi();const root=document.getElementById('sessions');const openResults=new Set();const resultCloseTimers=new Map();const selectedRepositoryGroupIds=new Set();let sessions=[];let authentication={status:'checking'};let draggedSessionId;let composingSessionId;let renderPending=false;
