...
export class SessionDetailPanel implements vscode.Disposable {
  ...
  private async handleMessage(sessionId: string, state: PanelState, value: unknown): Promise<void> {
    ...
    try {
      ...
      else if (message.type === "revealLocalFile" && typeof message.url === "string") {
        const localPath = localFilePathFromLink(message.url);
        if (localPath) await vscode.commands.executeCommand("revealFileInOS", vscode.Uri.file(localPath));
      }
...
export function localFilePathFromLink(value: string): string | undefined {
  if (!value || value.length > 8192) return undefined;
  let decoded: string;
  try { decoded = decodeURIComponent(value); } catch { return undefined; }
  if (/^[a-zA-Z]:[\\/]/.test(decoded)) return path.win32.normalize(decoded);
  if (/^\\\\[^\\/]+[\\/][^\\/]+/.test(decoded)) return path.win32.normalize(decoded);
  return undefined;
}
