...
export class RepositoryWebviewProvider implements vscode.WebviewViewProvider {
  ...
  public constructor(private readonly manager: RepositoryManager, private readonly addRepository: () => Promise<void>, private readonly openRepository: (repositoryId: string) => Promise<void>, private readonly createSession: (repositoryId: string) => Promise<void>, private readonly onError: (error: unknown) => void) {}
...
function renderHtml(groups: RepositoryGroupSnapshot[]): string {
  ...
  return `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}'; script-src 'nonce-${nonce}';"><style nonce="${nonce}">*{box-sizing:border-box}body{padding:8px;color:var(--vscode-foreground);background:var(--vscode-sideBar-background);font-family:var(--vscode-font-family);font-size:var(--vscode-font-size)}header{display:flex;gap:6px;margin-bottom:8px}header button{flex:1;border:0;padding:6px;color:var(--vscode-button-foreground);background:var(--vscode-button-background)}.groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}article{display:flex;min-width:0;flex-direction:column;border:1px solid var(--vscode-panel-border)}.group-row{display:flex;flex:1}.group{min-width:0;flex:1;padding:8px;text-align:left;border:0;color:var(--vscode-foreground);background:transparent}.group:hover,.group:focus-visible{background:var(--vscode-list-hoverBackground);outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.group strong,.group span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.group span{margin-top:3px;color:var(--vscode-descriptionForeground);font-size:11px}.remove{flex:none;width:28px;border:0;color:var(--vscode-foreground);background:transparent}.remove:hover{background:var(--vscode-toolbar-hoverBackground)}.create-session{width:100%;padding:6px 8px;border:0;border-top:1px solid var(--vscode-panel-border);color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground);text-align:left}.create-session:hover{background:var(--vscode-button-secondaryHoverBackground)}.create-session:focus-visible{outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.empty{padding:8px;color:var(--vscode-descriptionForeground);font-size:11px}@media(max-width:420px){.groups{grid-template-columns:1fr}}</style></head><body><header><button id="add">＋ フォルダを登録</button><button id="refresh" title="更新">↻</button></header>${content}<script nonce="${nonce}">const vscode=acquireVsCodeApi();document.getElementById('add').addEventListener('click',()=>vscode.postMessage({type:'add'}));document.getElementById('refresh').addEventListener('click',()=>vscode.postMessage({type:'refresh'}));document.querySelectorAll('[data-open]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'open',repositoryId:button.dataset.open})));document.querySelectorAll('[data-create-session]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'createSession',repositoryId:button.dataset.createSession})));document.querySelectorAll('[data-remove]').forEach(button=>button.addEventListener('click',()=>vscode.postMessage({type:'remove',repositoryId:button.dataset.remove})));</script></body></html>`;
}
